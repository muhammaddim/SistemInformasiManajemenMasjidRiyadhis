...
<tr>
    <td>{$i}</td>
    <td>{$judul}</td>
    <td>{$jamawal} s/d {$jamakhir}</td>
    <td>{$tglawal} s/d {$tglakhir}</td>
    <td>{$ket}</td>
    <td>
        <a href='edit-agenda.php?id={$id}' class='btn btn-sm btn-warning'>Edit</a>
        <a href='contents/hapus-agenda.php?id={$id}' class='btn btn-sm btn-danger' onclick='return confirm(\"Yakin ingin menghapus agenda ini?\")'>Hapus</a>
    </td>
</tr>";
$i++;
}
